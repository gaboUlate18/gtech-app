...
def generar_pdf_orden(datos):
    ...
    campos = [("ID Orden:", datos['n_orden']), ("Cliente:", datos['cliente']), 
              ("Material:", datos['material']), ("Etapa Actual:", datos.get('etapa', 'Registro'))]
    for k, v in campos:
...
def generar_pdf_tabla_historial(df):
    ...
    pdf.cell(0, 10, "REPORTE DE HISTORIAL GENERAL - G-TECH", ln=True, align='C')
    pdf.ln(10)
    pdf.set_font("Arial", 'B', 10); pdf.set_fill_color(200, 200, 200)
    headers = ["Orden", "Cliente", "Material", "Etapa", "Estado"]
    widths = [30, 70, 70, 50, 40]
    for i, h in enumerate(headers):
        pdf.cell(widths[i], 10, h, border=1, fill=True)
    pdf.ln()
    pdf.set_font("Arial", size=9)
    for _, row in df.iterrows():
        pdf.cell(30, 8, str(row['n_orden']), border=1)
        pdf.cell(70, 8, str(row['cliente'])[:35], border=1)
        pdf.cell(70, 8, str(row['material'])[:35], border=1)
        pdf.cell(50, 8, str(row['etapa']), border=1)
...
st.markdown("""
    ...
    .main-title { color: #0e4b7a; text-align: center; font-weight: 800; font-size: 3.5rem; margin-top: 50px; }
    </style>
...
if not st.session_state.auth:
    ...
    with t1:
        if st.session_state.registro_ok:
            ...
            with st.form("form_reg"):
                ...
                f_out = c2.date_input("Fecha Entrega")
                up = st.file_uploader("Plano Técnico", type=['pdf','png','jpg'])
                if st.form_submit_button("💾 GUARDAR"):
                    if up is None: st.error("Archivo requerido.")
                    else:
                        if ordenes_col.find_one({"n_orden": n_id}):
                            st.error("ID duplicada."); st.session_state.id_proxima = obtener_primera_id_desocupada(); st.rerun()
                        else:
                            d = {"n_orden": n_id, "cliente": cli, "f_recepcion": str(f_in), "material": mat, 
                                 "cantidad": cant, "entrega": str(f_out), "etapa": "Registro", "status": "Pendiente", "historial": []}
    ...
    with t2:
        ...
        if activas:
            with st.form("form_traz"):
                ...
                et_lista = ["Registro", "Maquinado", "Rectificado", "Tratamiento", "Ensamble", "Calidad", "Pulido", "Finalizado"]
                et = st.selectbox("Nueva Etapa", et_lista)
                h1, h2 = st.columns(2)
                t_i, t_f = h1.time_input("Inicio"), h2.time_input("Fin")
                obs = st.text_area("Notas de Proceso")
                if st.form_submit_button("ACTUALIZAR"):
                    st_f = "Finalizado" if et == "Finalizado" else "Pendiente"
                    ordenes_col.update_one({"n_orden": sel}, {"$set": {"etapa": et, "status": st_f}, 
                                           "$push": {"historial": {"etapa": et, "inicio": str(t_i), "fin": str(t_f), "nota": obs}}})
                    st.success("Proceso actualizado."); time.sleep(1); st.rerun()
    ...
    with t3:
        ...
        if todo:
            ...
            # CORRECCIÓN DE ERROR .applymap -> .map
            def color_status(val):
                color = '#d4edda' if val == 'Finalizado' else '#f8d7da'
